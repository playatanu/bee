...
- [`tests/vm_diff_test.sh`](../tests/vm_diff_test.sh) runs every example and 16
...
- [`tests/perf_guard_test.sh`](../tests/perf_guard_test.sh) checks *complexity*,
...
([`jit_llvm.cpp:297`](../src/jit_llvm.cpp#L297)).

### …but any real code leaves the subset

Codegen throws `JitBail` on the first construct outside the numeric subset
([`jit_llvm.cpp:301`](../src/jit_llvm.cpp#L301) for statements,
[`jit_llvm.cpp:412`](../src/jit_llvm.cpp#L412) for expressions). That means a
...
[`interpreter.cpp:844`](../src/interpreter.cpp#L844),
[`:878`](../src/interpreter.cpp#L878),
[`:880`](../src/interpreter.cpp#L880):
...
caught by every loop ([`:772`](../src/interpreter.cpp#L772),
[`:788`](../src/interpreter.cpp#L788),
[`:806`](../src/interpreter.cpp#L806)) and by `callFunction`
([`:1755`](../src/interpreter.cpp#L1755)).
...
[`:753`](../src/interpreter.cpp#L753) (block),
[`:783`](../src/interpreter.cpp#L783) (for),
[`:800`](../src/interpreter.cpp#L800) (for-in),
[`:1733`](../src/interpreter.cpp#L1733) (call frame) all do:
...
member ([`environment.hpp:28`](../src/environment.hpp#L28)) that a function frame
never uses. Hence the 62 ns cost of a single `let` in a loop body.

### 3. Value representation

`Value` ([`value.hpp:116`](../src/value.hpp#L116)) is a 24-byte `std::variant`
...
- `asList()` / `asDict()` / `asString()` return the `shared_ptr` **by value**
  ([`value.hpp:167-174`](../src/value.hpp#L167-L174)), so a single `xs[i]` does
  roughly four atomic refcount read-modify-writes.
- The refcounts are atomic even though Bee has a GIL
  ([`interpreter.hpp:115`](../src/interpreter.hpp#L115)) — the synchronisation is
  paid for and never used.

### 4. Per-call and per-property overhead

- [`callFunction:1684`](../src/interpreter.cpp#L1684) builds a `std::string name`
...
- `evalCall` ([`:1629`](../src/interpreter.cpp#L1629)) heap-allocates a
  `std::vector<Value> args` per call.
- `getProperty` ([`:1779`](../src/interpreter.cpp#L1779)) dispatches type methods
...
Two changes, both in [`resolver.cpp`](../src/resolver.cpp) and
[`interpreter.cpp`](../src/interpreter.cpp):
...
- [`chunk.hpp`](../src/chunk.hpp) — 8-byte three-operand instructions, and the
  opcode list as an X-macro so the enum and the VM's jump table are generated
  from one source and cannot drift.
- [`compiler.cpp`](../src/compiler.cpp) — AST to bytecode. Declines a function it
...
- [`vm.cpp`](../src/vm.cpp) — computed-goto dispatch (`&&label` / `goto *`), so
...
**public native-module ABI** ([`bee_native.hpp`](../src/bee_native.hpp)). Changing
...
- [ ] **Method dispatch:** replace the `name == "…"` chain in `getProperty`
      ([`:1779`](../src/interpreter.cpp#L1779)) with a per-type hash table, and
      cache the resolved method on the `GetExpr` AST node (an inline cache, like
      the one already used for globals in [`ast.hpp:51`](../src/ast.hpp#L51)).
...
- **`ForIn` over a list** copies each element into the loop slot; for large lists
  ...
  avoid.
- **Dicts** are `std::map` ([`value.hpp:20`](../src/value.hpp#L20)) — a red-black
