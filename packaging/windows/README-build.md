...
This produces a single double-click `bee-0.3.1-amd64.exe` that installs Bee for
...
g++ -std=c++17 -O2 -pthread -DBEE_VERSION='\"0.3.1\"' (Get-ChildItem src\*.cpp).FullName -o packaging\windows\bee.exe -pthread

# 2. Compile the package manager (no LLVM either).
g++ -std=c++17 -O2 -pthread -DHIVE_VERSION='\"0.3.1\"' (Get-ChildItem src\hive\*.cpp).FullName -o packaging\windows\hive.exe -pthread

# 3. Compile the binding generator. It loads libclang at run time, so there is
#    nothing to link against here.
g++ -std=c++17 -O2 -pthread -DBEEGEN_VERSION='\"0.3.1\"' (Get-ChildItem src\beegen\*.cpp).FullName -o packaging\windows\beegen.exe -pthread
...
The result is written to `dist\bee-0.3.1-amd64.exe`.
