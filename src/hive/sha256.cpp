// SHA-256 (FIPS 180-4), so integrity checks need no external crypto library.
#include "hive.hpp"
...
struct Sha256 {
    ...
    void finish(uint8_t out[32]) {
        ...
        // Feed the length directly: update() would corrupt `bits`, which we
        // already captured above.
        std::memcpy(buf + buffered, len, 8);
        compress(buf);
        buffered = 0;
        for (int i = 0; i < 8; ++i) {
