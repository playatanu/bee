...
std::string emitNativeSource(const Options& opts, const Api& api) {
    ...
    o << "// Generated by beegen -- do not edit.\n"
      << "//\n"
      << "// A native module for BeeLang, binding:\n";
...
// Bee side
// ---------------------------------------------------------------------------
// One BeeLang function per C++ function. When the C++ side has default
...
std::string emitBeeWrapper(const Options& opts, const Api& api) {
    ...
    for (auto& cls : api.classes) {
        ...
        for (auto& fn : cls.methods) {
            // A static C++ method has no instance to hang off, and BeeLang
        ...
        for (auto& fn : cls.methods) {
            if (!fn.isStatic) continue;
            if (!wroteStatic) {
                o << "\n# " << cls.cxxName << "'s static methods: BeeLang classes have no\n"
...
std::string emitBuildScript(const Options& opts, const Api& api) {
    ...
    o << "#!/usr/bin/env bash\n"
      << "# Generated by beegen -- builds the native module for BeeLang.\n"
      << "#\n"
...
std::string emitManifest(const Options& opts) {
    ...
    o << "{\n"
      << "  \"name\": \"" << opts.module << "\",\n"
      << "  \"version\": \"0.1.0\",\n"
      << "  \"description\": \"Generated BeeLang bindings\",\n"
      << "  \"license\": \"MIT\",\n"
