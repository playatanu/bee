...
A couple of things still need the [interpreter](getting-started.md) - `beec`
reports them (with a file and line) instead of miscompiling:
...
!!! note "About speed"
    A compiled binary is standalone, but it is not automatically *faster* than
    the interpreter. Arithmetic still flows through Bee's dynamic runtime, and
    the interpreter already compiles hot numeric loops to native code with its
    [JIT](performance.md). Making AOT output match or beat the JIT on numeric
    code (by specialising on types) is planned work, not a promise today.
...
- **Windows:** run `beec` from an MSYS2 / MinGW shell, whose `g++` is the same
...
- On a relocated install, point `beec` at the runtime with `BEE_AOT_INCDIR` (the
  directory with `bee_aot.hpp`) and `BEE_AOT_RUNTIME_LIB` (the path to
  `libbee_runtime.a`).
